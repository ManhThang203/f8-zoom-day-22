...
todoForm.onsubmit = (event) => {
  ...
  const isDuplicate = todoTask.some(
    (task, index) =>
      // So sánh tiêu đề (không phân biệt chữ hoa/thường)
      // và đảm bảo rằng khi chỉnh sửa, không tự so sánh với chính nó
      task.title.toLowerCase() === fromValue.title.toLowerCase() &&
      (editIndex === null || index !== parseInt(editIndex))
  );
...
todoList.onclick = (event) => {
  ...
  // Biến lưu index nhiệm vụ chờ xoá
  let taskIndexToDelete = null;

  // Lấy các nút xác nhận xoá
  const btnCancelDelete = $(".btn-canncel"); // Nút Cancel
  console.log(btnCancelDelete);
  const btnYesDelete = $(".btn-yes"); // Nút Yes
  // bắt sự kiện khi nhấn vào nút xóa
  const deleteBtn = event.target.closest(".delete-btn");

  // Xử lý khi nhấn Delete
  if (deleteBtn) {
    taskIndexToDelete = deleteBtn.dataset.index;
      ...
      (task, index) => `
        ...
        task.isCompleted ? "completed" : ""
                    ...
                    <div class="task-time">${EscapeHTML(
                      task.startTime
                    )} - ${EscapeHTML(task.endTime)}</div>
           </div>`
    )
    .join(""); // Nối các phần tử HTML thành một chuỗi
...
tabButtons.forEach((button) => {
  button.addEventListener("click", (event) => {
    ...
    if (buttonText === "All Task") {
      ...
      // Kiểm tra nếu nút chứa từ "Completed"
